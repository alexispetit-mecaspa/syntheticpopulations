...
if __name__ == "__main__": 
  ...
  print 'initial correlation between RAAN and i', np.corrcoef(raan_init,inc_init)
  ...
  controls,laws = read_controls('simulation1.txt',var_list,limits,nb_obj,change_n_obj)
  print '> ',controls
  print 
  ...
  ax.plot(raan,inc,'o',markersize=4,color='darkorange',label='Synthetic population ('+str(len(raan))+' fragments)',alpha=0.7)
  for j in range(0,n_dim['i[deg]'],1):
    x = limits['i[deg]'][j]
  ...
  plt.subplot(1, 2, 1)
  x = np.linspace(295, 330, 100)
  p = norm.pdf(x,laws['RAAN[deg]'][0],laws['RAAN[deg]'][1])
  plt.plot(x,p,'k',linewidth=2, label="Assumed distribution")
  plt.hist([a[0] for a in raan], histtype='stepfilled',color="orange", alpha=0.6, normed=True, bins=40, label="Synthetic population")
  plt.hist(raan_init, histtype='stepfilled', alpha=0.4, normed=True, bins=40, label="Simulation")
  plt.xlabel('RAAN [deg]')
  plt.ylabel('Density')
  plt.legend(loc='upper left',fontsize=12)
  plt.ylim((0,0.35))
  plt.subplot(1, 2, 2)
  plt.hist([a[0] for a in inc], histtype='stepfilled',color="orange", alpha=0.6, normed=True, bins=40, label="Synthetic population")
  plt.hist(inc_init, histtype='stepfilled', alpha=0.4, normed=True, bins=40, label="Simulation")
  plt.xlabel('i[deg]')
  x = np.linspace(8, 16, 100)
  p = norm.pdf(x,laws['i[deg]'][0],laws['i[deg]'][1])
  plt.plot(x,p,'k',linewidth=2, label="Normal distribution")
  plt.ylim((0,0.9))
  plt.savefig('Distributions_pop')
  plt.close()
